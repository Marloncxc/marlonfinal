...
def check_login(username, password):
    users = {"marloncxc": "@Sayro10/", "user1": "pass456"}
    return users.get(username) == password

# Funçao da tela de login
def login_screen():
    st.title("Login")
...
# Verificar se o usuario esta logado e redirecionao para a tela apropriada
if not st.session_state["logged_in"]:
    login_screen()
else:

    # Código a ser exibido após login bem-sucedido
    ...
    if st.sidebar.button('Gerar Relatório'):
        relatorio_diario = movimentacao[movimentacao['Data'].dt.date == data_relatorio]
